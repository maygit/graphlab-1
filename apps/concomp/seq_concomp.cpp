...
void concomp_update(gl_types::iscope &scope,
	...
	vid_t nlabel;	
	foreach(graphlab::edge_id_t eid, scope.in_edge_ids()){
		nlabel = scope.edge_data(eid).weight.load(std::memory_order_seq_cst);	
		curmin = std::min(curmin, nlabel);
	}
	foreach(graphlab::edge_id_t eid, scope.out_edge_ids()){
		nlabel = scope.edge_data(eid).weight.load(std::memory_order_seq_cst);
	...
	double residual = 100;		
	foreach(graphlab::edge_id_t eid, scope.out_edge_ids()){
		if(curmin < scope.edge_data(eid).weight.load(std::memory_order_seq_cst)){
			...
			//edge_data& edata = scope.edge_data(eid);
			scope.edge_data(eid).weight.store(curmin , std::memory_order_seq_cst);
