...
typedef std::atomic<vid_t> edge_data;
/*
struct edge_data {
  std::atomic<vid_t> weight;
...
void concomp_init(gl_types::iscope &scope,
	...
	vdata = scope.vertex();//initiate vertex data with its id;
	//vid_t tmp = vdata;
	vid_t curmin = vdata;//scope.vertex();
...
void concomp_update(gl_types::iscope &scope,
	...
	vid_t nlabel;	
	foreach(graphlab::edge_id_t eid, scope.in_edge_ids()){
		nlabel = scope.get_edge_data(eid);//.weight.load(std::memory_order_ralexed);	
		curmin = std::min(curmin, nlabel);
	}
	foreach(graphlab::edge_id_t eid, scope.out_edge_ids()){
		nlabel = scope.get_edge_data(eid);//.weight.load(std::memory_order_relaxed);
		curmin = std::min(curmin, nlabel);
	}
	/*
	   foreach(graphlab::edge_id_t eid, scope.in_edge_ids()){
	...
	double residual = 100;		
	foreach(graphlab::edge_id_t eid, scope.out_edge_ids()){
		if(curmin < (scope.get_edge_data(eid))/*.weight.load(std::memory_order_relaxed)*/){
			...
			//edge_data& edata = scope.edge_data(eid);
			//(scope.edge_data(eid).weight).store(curmin , std::memory_order_relaxed);
			scope.set_edge_data(eid , curmin);
	...
	foreach(graphlab::edge_id_t eid, scope.in_edge_ids()){
		if(curmin < (scope.edge_data(eid))/*.weight.load(std::memory_order_relaxed)*/){
...
int main(int argc, char** argv) {
	...
	std::cout << "async wcc finished,engine runtime: " << runtime << " seconds." << std::endl;
	std::cout << "async wcc total updates count "<< core.last_update_count() <<std::endl;
